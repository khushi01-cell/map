...
def find_label_near_centroid(msp, centroid, target_label="26", max_distance=100):
...
def calculate_plot_26_area(file_path):
    """Calculate plot 26 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 26 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 26 candidates
    all_polygons = []
    plot_26_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 26 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "26")
        ...
        if has_label:
            plot_26_candidates.append(polygon_info)
            print(f"  Found plot 26 candidate: {area_raw:.2f} DXF units")
    ...
    plot_26_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 26 candidates: {len(plot_26_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_26_candidates:
        # Use the plot 26 candidate with closest label
        selected_polygon = plot_26_candidates[0]
        print(f"\n✅ SELECTED: Plot 26 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 26 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '26' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_26_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 26 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 34381.91
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 26 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 26 area") 