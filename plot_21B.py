...
def find_label_near_centroid(msp, centroid, target_label="21/B", max_distance=100):
        ...
        if entity.dxftype() in ("TEXT", "MTEXT"):
            try:
                ...
                if distance <= max_distance:
                    ...
                    if text_value in ["21/B", "21B", "21/B", "21/B"]:
                        return True, distance, text_value
            except Exception:
                continue
    return False, 0, ""

def calculate_plot_21B_area(file_path):
    """Calculate plot 21/B area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 21/B AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 21/B candidates
    all_polygons = []
    plot_21B_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 21/B label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "21/B")
        ...
        if has_label:
            plot_21B_candidates.append(polygon_info)
            print(f"  Found plot 21/B candidate: {area_raw:.2f} DXF units")
    ...
    plot_21B_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 21/B candidates: {len(plot_21B_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_21B_candidates:
        # Use the plot 21/B candidate with closest label
        selected_polygon = plot_21B_candidates[0]
        print(f"\n✅ SELECTED: Plot 21/B with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 21/B label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '21/B' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_21B_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 21/B Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 25096.90
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 21/B Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 21/B area") 