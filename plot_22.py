...
def find_label_near_centroid(msp, centroid, target_label="22", max_distance=100):
...
def calculate_plot_22_area(file_path):
    """Calculate plot 22 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 22 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 22 candidates
    all_polygons = []
    plot_22_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 22 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "22")
        ...
        if has_label:
            plot_22_candidates.append(polygon_info)
            print(f"  Found plot 22 candidate: {area_raw:.2f} DXF units")
    ...
    plot_22_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 22 candidates: {len(plot_22_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_22_candidates:
        # Use the plot 22 candidate with closest label
        selected_polygon = plot_22_candidates[0]
        print(f"\n✅ SELECTED: Plot 22 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 22 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '22' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_22_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 22 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 18491.12
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 22 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 22 area") 