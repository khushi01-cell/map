...
def find_label_near_centroid(msp, centroid, target_label="16", max_distance=100):
...
def calculate_plot_16_area(file_path):
    """Calculate plot 16 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 16 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 16 candidates
    all_polygons = []
    plot_16_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 16 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "16")
        ...
        if has_label:
            plot_16_candidates.append(polygon_info)
            print(f"  Found plot 16 candidate: {area_raw:.2f} DXF units")
    ...
    plot_16_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 16 candidates: {len(plot_16_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_16_candidates:
        # Use the plot 16 candidate with closest label
        selected_polygon = plot_16_candidates[0]
        print(f"\n✅ SELECTED: Plot 16 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 16 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '16' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_16_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 16 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 478.29
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 16 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 16 area") 