...
def find_label_near_centroid(msp, centroid, target_label="15/A", max_distance=100):
        ...
        if entity.dxftype() in ("TEXT", "MTEXT"):
            try:
                ...
                if distance <= max_distance:
                    ...
                    if text_value in ["15/A", "15A", "15/A", "15/A"]:
                        return True, distance, text_value
            except Exception:
                continue
    return False, 0, ""

def calculate_plot_15A_area(file_path):
    """Calculate plot 15/A area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 15/A AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 15/A candidates
    all_polygons = []
    plot_15A_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 15/A label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "15/A")
        ...
        if has_label:
            plot_15A_candidates.append(polygon_info)
            print(f"  Found plot 15/A candidate: {area_raw:.2f} DXF units")
    ...
    plot_15A_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 15/A candidates: {len(plot_15A_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_15A_candidates:
        # Use the plot 15/A candidate with closest label
        selected_polygon = plot_15A_candidates[0]
        print(f"\n✅ SELECTED: Plot 15/A with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 15/A label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '15/A' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_15A_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 15/A Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 1807.30
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 15/A Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 15/A area") 