...
def find_label_near_centroid(msp, centroid, target_label="19", max_distance=100):
...
def calculate_plot_19_area(file_path):
    """Calculate plot 19 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 19 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 19 candidates
    all_polygons = []
    plot_19_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 19 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "19")
        ...
        if has_label:
            plot_19_candidates.append(polygon_info)
            print(f"  Found plot 19 candidate: {area_raw:.2f} DXF units")
    ...
    plot_19_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 19 candidates: {len(plot_19_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_19_candidates:
        # Use the plot 19 candidate with closest label
        selected_polygon = plot_19_candidates[0]
        print(f"\n✅ SELECTED: Plot 19 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 19 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '19' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_19_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 19 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 154.68
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 19 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 19 area") 