...
def find_label_near_centroid(msp, centroid, target_label="42", max_distance=20):
...
def calculate_plot_42_area(file_path):
    """Calculate plot 42 area using green boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    # Reference area for plot 42
    reference_area = 103.68

    print("🔍 CALCULATING PLOT 42 AREA (GREEN LINES ONLY)")
    print("=" * 60)
    
    # First, find the raw area of plot 42 to calculate the correct conversion factor
    raw_area = None
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for text label "42" near centroid
        has_label, distance = find_label_near_centroid(msp, centroid, "42")
        
        if has_label:
            raw_area = polygon_area(pts)
            print(f"  Found plot 42 with raw area: {raw_area:.6f} DXF units")
            break
    
    # If plot 42 not found by label, use first green boundary
    if raw_area is None:
        print("  Plot 42 not found by label, using first green boundary...")
        for entity in msp.query("LWPOLYLINE"):
    ...
    # Find plot 42 by looking for polygon with text label "42"
    plot_42_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        if has_label:
            plot_42_candidates.append((area_m2, area_raw, pts, distance, centroid, entity.dxf.color))
            print(f"  Found plot 42 candidate:")
    ...
    if plot_42_candidates:
        # Sort by distance to text (closest first)
        plot_42_candidates.sort(key=lambda x: x[3])
        best_candidate = plot_42_candidates[0]
        
        area_m2, area_raw, pts, distance, centroid, color = best_candidate
        
        print(f"\n✅ ORIGINAL PLOT 42 FOUND!")
        print(f"  Area: {area_m2:.2f} sq.meter")
        print(f"  Raw area (DXF units): {area_raw:.2f}")
        print(f"  Conversion factor: {conversion_factor}")
        print(f"  Color: {color}")
        print(f"  Distance to text '42': {distance:.2f}")
        print(f"  Points: {len(pts)}")
        
        return area_m2
    else:
        # If no plot 42 found by label, try to find green boundaries
        print("  No plot 42 found by label, searching green boundaries...")
        ...
        if green_boundaries:
            # Return the first green boundary as plot 42
            area_m2, area_raw, pts, index = green_boundaries[0]
            print(f"\n✅ ORIGINAL PLOT 42 (Green boundary #{index}):")
            print(f"  Area: {area_m2:.2f} sq.meter")
            print(f"  Raw area (DXF units): {area_raw:.2f}")
            print(f"  Conversion factor: {conversion_factor}")
            print(f"  Points: {len(pts)}")
...
area = calculate_plot_42_area(file_path)

if area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 42 Area: {area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 42 area")
