...
def find_label_near_centroid(msp, centroid, target_label="45", max_distance=100):
...
def calculate_plot_45_area(file_path):
    """Calculate plot 45 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 45 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 45 candidates
    all_polygons = []
    plot_45_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 45 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "45")
        ...
        if has_label:
            plot_45_candidates.append(polygon_info)
            print(f"  Found plot 45 candidate: {area_raw:.2f} DXF units")
    ...
    plot_45_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 45 candidates: {len(plot_45_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_45_candidates:
        # Use the plot 45 candidate with closest label
        selected_polygon = plot_45_candidates[0]
        print(f"\n✅ SELECTED: Plot 45 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 45 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '45' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_45_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 45 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 66.05
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 45 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 45 area") 