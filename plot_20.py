...
def find_label_near_centroid(msp, centroid, target_label="20", max_distance=100):
...
def calculate_plot_20_area(file_path):
    """Calculate plot 20 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 20 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 20 candidates
    all_polygons = []
    plot_20_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 20 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "20")
        ...
        if has_label:
            plot_20_candidates.append(polygon_info)
            print(f"  Found plot 20 candidate: {area_raw:.2f} DXF units")
    ...
    plot_20_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 20 candidates: {len(plot_20_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_20_candidates:
        # Use the plot 20 candidate with closest label
        selected_polygon = plot_20_candidates[0]
        print(f"\n✅ SELECTED: Plot 20 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 20 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '20' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_20_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 20 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 173.08
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 20 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 20 area") 