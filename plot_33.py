...
def find_label_near_centroid(msp, centroid, target_label="33", max_distance=100):
...
def calculate_plot_33_area(file_path):
    """Calculate plot 33 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 33 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 33 candidates
    all_polygons = []
    plot_33_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 33 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "33")
        ...
        if has_label:
            plot_33_candidates.append(polygon_info)
            print(f"  Found plot 33 candidate: {area_raw:.2f} DXF units")
    ...
    plot_33_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 33 candidates: {len(plot_33_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_33_candidates:
        # Use the plot 33 candidate with closest label
        selected_polygon = plot_33_candidates[0]
        print(f"\n✅ SELECTED: Plot 33 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 33 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '33' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_33_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 33 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 12099.97
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 33 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 33 area") 