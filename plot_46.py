...
def find_label_near_centroid(msp, centroid, target_label="46", max_distance=100):
...
def calculate_plot_46_area(file_path):
    """Calculate plot 46 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 46 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 46 candidates
    all_polygons = []
    plot_46_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 46 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "46")
        ...
        if has_label:
            plot_46_candidates.append(polygon_info)
            print(f"  Found plot 46 candidate: {area_raw:.2f} DXF units")
    ...
    plot_46_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 46 candidates: {len(plot_46_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_46_candidates:
        # Use the plot 46 candidate with closest label
        selected_polygon = plot_46_candidates[0]
        print(f"\n✅ SELECTED: Plot 46 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 46 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '46' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_46_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 46 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 36.79
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 46 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 46 area") 