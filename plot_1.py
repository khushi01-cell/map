...
def find_label_near_centroid(msp, centroid, target_label="1", max_distance=100):
...
def calculate_plot_1_area(file_path):
    """Calculate plot 1 area using boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    print("🔍 CALCULATING PLOT 1 AREA (BOUNDARY LINES)")
    print("=" * 60)
    
    # Find all polygons and plot 1 candidates
    all_polygons = []
    plot_1_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 1 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "1")
        ...
        if has_label:
            plot_1_candidates.append(polygon_info)
            print(f"  Found plot 1 candidate: {area_raw:.2f} DXF units")
    ...
    plot_1_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Total polygons found: {len(all_polygons)}")
    print(f"Plot 1 candidates: {len(plot_1_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_1_candidates:
        # Use the plot 1 candidate with closest label
        selected_polygon = plot_1_candidates[0]
        print(f"\n✅ SELECTED: Plot 1 with label (closest)")
    elif all_polygons:
        # Use the largest polygon as fallback
        selected_polygon = all_polygons[0]
        print(f"\n✅ SELECTED: Largest polygon (no plot 1 label found)")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '1' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_1_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 1 Raw Area: {raw_area:.2f} DXF units")
    
    # Reference area
    reference_area = 325.17
    ...
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 1 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 1 area") 