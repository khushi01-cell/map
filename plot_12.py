...
def find_label_near_centroid(msp, centroid, target_label="12", max_distance=100):
...
def calculate_plot_12_area(file_path):
    """Calculate plot 12 area using green boundary lines"""
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    # Reference area for plot 12
    reference_area = 754.07

    print("🔍 CALCULATING PLOT 12 AREA (GREEN LINES ONLY)")
    print("=" * 60)
    
    # Find all green polygons and plot 12 candidates
    green_polygons = []
    plot_12_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for plot 12 label
        has_label, distance, label_text = find_label_near_centroid(msp, centroid, "12")
        ...
        if is_green_color:
            green_polygons.append(polygon_info)
            print(f"  Found green polygon: {area_raw:.2f} DXF units")
            print(f"    Color: {entity.dxf.color}, Layer: {entity.dxf.layer}")
            print(f"    Has '12' label: {has_label}")
            if has_label:
                print(f"    Distance to label: {distance:.2f}")
        
        if has_label:
            plot_12_candidates.append(polygon_info)
            print(f"  Found plot 12 candidate: {area_raw:.2f} DXF units")
            print(f"    Color: {entity.dxf.color} (Green: {is_green_color})")
            print(f"    Layer: {entity.dxf.layer}")
            print(f"    Distance to label: {distance:.2f}")

    # Sort by area (largest first)
    green_polygons.sort(key=lambda x: x['area'], reverse=True)
    plot_12_candidates.sort(key=lambda x: x['distance'])  # Closest label first

    print(f"\n📊 SUMMARY:")
    print(f"Green polygons found: {len(green_polygons)}")
    print(f"Plot 12 candidates: {len(plot_12_candidates)}")

    # Select the best candidate
    selected_polygon = None
    
    if plot_12_candidates:
        # Use the plot 12 candidate with closest label
        selected_polygon = plot_12_candidates[0]
        print(f"\n✅ SELECTED: Plot 12 with label (closest)")
    elif green_polygons:
        # Use the largest green polygon
        selected_polygon = green_polygons[0]
        print(f"\n✅ SELECTED: Largest green polygon")
    else:
    ...
    if selected_polygon['has_label']:
        print(f"  Distance to '12' label: {selected_polygon['distance']:.2f}")
    
    return selected_polygon['area']

# Calculate area
file_path = "CTP01(LALDARWAJA)FINAL.dxf"
raw_area = calculate_plot_12_area(file_path)

if raw_area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 12 Raw Area: {raw_area:.2f} DXF units")
    ...
    # Final calculation with exact conversion
    final_area = raw_area * exact_conversion
    print(f"\n🎯 FINAL CALCULATION:")
    print(f"Plot 12 Area: {final_area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 12 area") 