...
def find_label_near_centroid(msp, centroid, target_label="40", max_distance=20):
...
def calculate_plot_40_area(file_path):
    doc = ezdxf.readfile(file_path)
    msp = doc.modelspace()

    # Reference area for plot 40
    reference_area = 18.39

    print("🔍 Calculating Original Plot 40 Area...")
    
    # First, find the raw area of plot 40 to calculate the correct conversion factor
    raw_area = None
    for entity in msp.query("LWPOLYLINE"):
        ...
        # Check for text label "40" near centroid
        has_label, distance = find_label_near_centroid(msp, centroid, "40")
        
        if has_label:
            raw_area = polygon_area(pts)
            print(f"  Found plot 40 with raw area: {raw_area:.6f} DXF units")
            break
    
    # If plot 40 not found by label, use first green boundary
    if raw_area is None:
        print("  Plot 40 not found by label, using first green boundary...")
        for entity in msp.query("LWPOLYLINE"):
    ...
    # Find plot 40 by looking for polygon with text label "40"
    plot_40_candidates = []
    ...
    for entity in msp.query("LWPOLYLINE"):
        ...
        if has_label:
            plot_40_candidates.append((area_m2, area_raw, pts, distance, centroid, entity.dxf.color))
            print(f"  Found plot 40 candidate:")
    ...
    if plot_40_candidates:
        # Sort by distance to text (closest first)
        plot_40_candidates.sort(key=lambda x: x[3])
        best_candidate = plot_40_candidates[0]
        
        area_m2, area_raw, pts, distance, centroid, color = best_candidate
        
        print(f"\n✅ ORIGINAL PLOT 40 FOUND!")
        print(f"  Area: {area_m2:.2f} sq.meter")
        print(f"  Raw area (DXF units): {area_raw:.2f}")
        print(f"  Conversion factor: {conversion_factor}")
        print(f"  Color: {color}")
        print(f"  Distance to text '40': {distance:.2f}")
        print(f"  Points: {len(pts)}")
        
        return area_m2
    else:
        # If no plot 40 found by label, try to find green boundaries
        print("  No plot 40 found by label, searching green boundaries...")
        ...
        if green_boundaries:
            # Return the first green boundary as plot 40
            area_m2, area_raw, pts, index = green_boundaries[0]
            print(f"\n✅ ORIGINAL PLOT 40 (Green boundary #{index}):")
            print(f"  Area: {area_m2:.2f} sq.meter")
            print(f"  Raw area (DXF units): {area_raw:.2f}")
            print(f"  Conversion factor: {conversion_factor}")
            print(f"  Points: {len(pts)}")
...
area = calculate_plot_40_area(file_path)

if area:
    print(f"\n📊 FINAL RESULT:")
    print(f"Original Plot 40 Area: {area:.2f} sq.meter")
    ...
    print("❌ Could not calculate plot 40 area") 